...
def normalize(text: str) -> str:
    ...
    Согласно методическим рекомендациям (search.md): единый регистр,
    унификация ё→е, удаление кавычек/дефисов/пунктуации и лишних пробелов.
    """
    if not text:
        return ""
...
# =========================================================================== #
#        Этап 4 (search.md): нечёткий поиск похожих названий (Fuzzy)          #
# =========================================================================== #
# Aho–Corasick находит только ТОЧНЫЕ совпадения после нормализации. Нечёткое
# сравнение (RapidFuzz, Damerau–Levenshtein) ловит опечатки и неизвестные
# варианты («стэнфорд» → «стэнфордд»). Включается опцией (для массового обхода
# дорого). Из текста формируются окна по 1..N слов и сравниваются со словарём.
try:
...
class FuzzyMatcher:
    ...
    def __init__(self, patterns: List[Pattern], *, threshold: float = 90.0,
                 min_len: int = 8, max_words: int = 5) -> None:
    ...
    def find(self, norm_text: str, exclude_norms: set) -> List[FuzzyMatch]:
        """Похожие фрагменты, отсутствующие среди точных совпадений."""
        ...
        for size in range(1, self.max_words + 1):
            for i in range(len(tokens) - size + 1):
                ...
                if len(window) < self.min_len:
                    continue
                best = process.extractOne(window, self.choices,
                                          scorer=fuzz.ratio, score_cutoff=self.threshold)
                if not best:
                    continue
                _, score, idx = best
                pat = self.entries[idx]
                if pat.norm in exclude_norms or pat.norm == window:
                    continue
